...
if (!fs.existsSync(keystorePath)) {
	console.log('No upload keystore found at keystore/finddel-upload.jks — skipping release signing setup (release build will be signed with the debug key).');
	process.exit(0);
}
...
if (!alias || !password) {
	throw new Error('keystore/finddel-upload.jks exists but ANDROID_UPLOAD_KEYSTORE_ALIAS / ANDROID_UPLOAD_KEYSTORE_PASSWORD are missing from .env');
}
